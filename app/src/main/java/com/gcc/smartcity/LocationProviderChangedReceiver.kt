...
//        intent.action?.matches("android.location.PROVIDERS_CHANGED".toRegex()) {}
        intent.action?.let { act ->
            if (act.matches("android.location.PROVIDERS_CHANGED".toRegex())) {
                ...
                if (!(isGpsEnabled && isNetworkEnabled)) {
                    AlertDialogBuilder.getInstance().showErrorDialog("LOCATION TURNED OFF","Please turn ON location to continue using the app","CANCEL", "OK", "", context, this)
                }
            }
        }
    }
